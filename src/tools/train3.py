...
def main(args):
    ...
    for epoch in range(epo, args.epoch):
        if epoch == epo: 
            args.logger.debug( f"Path: {args.output_dir} | Dataset_len: {len(train_dataset)} | Type: {d_type} | View: {args.view} | Dataset: {args.dataset} | Model: {args.model} | Status: {args.reset} | 2D_loss: {args.loss_2d} | Max_count : {args.count} | Max_epoch : {args.epoch}")
            print(colored(f"Path: {args.output_dir} | Dataset_len: {len(train_dataset)} | Type: {d_type} | View: {args.view} | Dataset: {args.dataset} | Model: {args.model} | Status: {args.reset} | 2D_loss: {args.loss_2d} | Max_count : {args.count} | Max_epoch : {args.epoch}", "yellow"))
